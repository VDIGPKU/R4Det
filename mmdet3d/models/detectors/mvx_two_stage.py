...
class MVXTwoStageDetector(Base3DDetector):
    ...
    def with_img_bbox(self):
        ...
        return hasattr(self,
                       'img_roi_head') and self.img_roi_head is not None#原本是bbox head
    ...
    def simple_test_rpn(self, x, img_metas, rpn_test_cfg):
        ...
        #proposal_list = self.img_rpn_head.get_bboxes(*proposal_inputs)
        proposal_list = self.img_rpn_head.get_bboxes(
            *rpn_outs,
            img_metas=img_metas,
            cfg=rpn_test_cfg)
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        outs = self.pts_bbox_head(x)#将bev转为框
        #[0]是类别分数；[1]是边界框相对于anchor的偏移值[批次大小 * anchor 数量, 7]
        bbox_list = self.pts_bbox_head.get_bboxes(
            *outs, img_metas, rescale=rescale)###修改了这里原本*outs
        bbox_results = [
