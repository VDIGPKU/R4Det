...
class PerceptionTransformer_DFA3D(PerceptionTransformer):
    def get_bev_features(
        ...
        if bev_coords.device != bev_queries.device:
            bev_coords = bev_coords.to(bev_queries.device)
        if bev_coords.device != bev_pos.device:
            bev_coords = bev_coords.to(bev_pos.device)
        unmasked_bev_queries = bev_queries[bev_coords[unmasked_idx, -1], :, :]
        unmasked_bev_bev_pos = bev_pos[bev_coords[unmasked_idx, -1], :, :]
        if bev_queries.device != ref_bev.device:
            ref_bev = ref_bev.to(bev_queries.device)
    ...
    def diffuse_vox_features(
        ...
        if bev_coords.device != ref_bev.device:
            ref_bev = ref_bev.to(bev_coords.device)
        unmasked_ref_bev = ref_bev[bev_coords[unmasked_idx, -1], :]
