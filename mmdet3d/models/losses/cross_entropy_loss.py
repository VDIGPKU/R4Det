...
def cross_entropy(pred,
    ...
    try:
        ...
        if 'CUDA error: device-side assert triggered' in str(e):
            # 如果是 CUDA 断言错误，打印详细信息
            print("\n" + "#" * 30 + " CUDA Assert Triggered in F.cross_entropy " + "#" * 30)
            print("!!! Catched CUDA device-side assert triggered! Printing debug info... !!!")

            # 打印输入 (pred/logits) 的信息
            print("\n--- Input 'pred' (Logits) ---")
            print(f"  Shape: {pred.shape}")
            print(f"  Dtype: {pred.dtype}")
            print(f"  Device: {pred.device}")
            if pred.numel() > 0:
                print(f"  Min value: {torch.min(pred).item()}")
                print(f"  Max value: {torch.max(pred).item()}")
                print(f"  Contains NaN: {torch.isnan(pred).any().item()}")
                print(f"  Contains Inf: {torch.isinf(pred).any().item()}")
            else:
                print("  Tensor is empty.")

            # 打印目标 (label/target) 的信息
            print("\n--- Target 'label' (Ground Truth Labels) ---")
            print(f"  Shape: {label.shape}")
            print(f"  Dtype: {label.dtype}")  # 应该是 LongTensor (int64)
            print(f"  Device: {label.device}")
            if label.numel() > 0:
                label_min = torch.min(label)
                label_max = torch.max(label)
                print(f"  Min label value: {label_min.item()}")
                print(f"  Max label value: {label_max.item()}")
                # 打印所有唯一值，帮助找到越界标签
                ...
                print(f"\n--- Validation Check ---")
                print(f"  Inferred number of classes (from pred shape): {num_classes}")
                print(f"  Expected label range: [0, {num_classes - 1}]")

                # 检查是否有越界标签
                if label_min < 0 or label_max >= num_classes:
                    print(f"  !!!!!! CRITICAL ERROR: Labels are OUT OF BOUNDS !!!!!!")
                    # 找出具体的越界标签
                    out_of_bounds_mask = (label < 0) | (label >= num_classes)
                    out_of_bounds_labels = label[out_of_bounds_mask]
                    print(f"  Offending labels: {torch.unique(out_of_bounds_labels)}")
                else:
                    print(f"  Label range seems VALID.")
            else:
