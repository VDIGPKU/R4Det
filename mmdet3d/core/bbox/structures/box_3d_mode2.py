...
from .lidar_box3d2 import LiDARInstance3DBoxes2


@unique
class Box3DMode2(IntEnum):
    ...
    def convert(box, src, dst, rt_mat=None):
        ...
        if src == Box3DMode2.LIDAR and dst == Box3DMode2.CAM:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[0, -1, 0], [0, 0, -1], [1, 0, 0]])
            xyz_size = torch.cat([y_size, z_size, x_size], dim=-1)
        elif src == Box3DMode2.CAM and dst == Box3DMode2.LIDAR:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[0, 0, 1], [-1, 0, 0], [0, -1, 0]])
            xyz_size = torch.cat([z_size, x_size, y_size], dim=-1)
        elif src == Box3DMode2.DEPTH and dst == Box3DMode2.CAM:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[1, 0, 0], [0, 0, 1], [0, -1, 0]])
            xyz_size = torch.cat([x_size, z_size, y_size], dim=-1)
        elif src == Box3DMode2.CAM and dst == Box3DMode2.DEPTH:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
            xyz_size = torch.cat([x_size, z_size, y_size], dim=-1)
        elif src == Box3DMode2.LIDAR and dst == Box3DMode2.DEPTH:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[0, -1, 0], [1, 0, 0], [0, 0, 1]])
            xyz_size = torch.cat([y_size, x_size, z_size], dim=-1)
        elif src == Box3DMode2.DEPTH and dst == Box3DMode2.LIDAR:
            if rt_mat is None:
                rt_mat = arr.new_tensor([[0, 1, 0], [-1, 0, 0], [0, 0, 1]])
            xyz_size = torch.cat([y_size, x_size, z_size], dim=-1)
        else:
        ...
        if is_numpy:
            ...
            if dst == Box3DMode2.CAM:
                target_type = CameraInstance3DBoxes
            elif dst == Box3DMode2.LIDAR:
                target_type = LiDARInstance3DBoxes2
            elif dst == Box3DMode2.DEPTH:
