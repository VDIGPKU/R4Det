...
class VoDDataset(Custom3DDataset):
    ...
    def get_ann_info(self, index):
        ...
        gt_labels = []  #car pedestrian cyclist truck
        for cat in gt_names:
    ...
    def format_results(self,
        ...
        num_samples_to_print = min(1, len(outputs))
        for i, sample_output in enumerate(outputs[:num_samples_to_print]):
            pts_bbox_data = sample_output['pts_bbox']
            boxes = pts_bbox_data['boxes_3d']
    ...
    def evaluate(self,
        ...
        eval_tmp_dir = './work_dirs/vod_eval_tmp_txts'
        if rank == 0 and os.path.exists(eval_tmp_dir):
            shutil.rmtree(eval_tmp_dir)
        submission_prefix = eval_tmp_dir + '/'
        result_files, tmp_dir = self.format_results(results, pklfile_prefix=pklfile_prefix,
                                                    submission_prefix=submission_prefix)
        ap_dict = dict()
        if rank == 0:
            pred_dir = None
            for root, dirs, files in os.walk(eval_tmp_dir):
            ...
            gt_dir = os.path.join(self.data_root, self.split, 'label_2')

            evaluation = Evaluation(test_annotation_file=gt_dir)
            vod_results = evaluation.evaluate(result_path=pred_dir, current_class=[0, 1, 2])

            log_str = "\n" + "=" * 65 + "\n"
            log_str += " " * 15 + "VoD Official Evaluation Results\n"
            log_str += "=" * 65

            regions = [('entire_area', 'Entire annotated area'),
                       ('roi', 'Driving corridor area'),
                       ('not_roi', 'NOT interested area far distance')]

            for region_key, region_name in regions:
                log_str += f"\n\n{region_name} | 3d bev aos:\n"
                log_str += "-" * 50 + "\n"
                res = vod_results[region_key]

                car_3d, car_bev, car_aos = res['Car_3d_all'], res['Car_bev_all'], res['Car_aos_all']
                ped_3d, ped_bev, ped_aos = res['Pedestrian_3d_all'], res['Pedestrian_bev_all'], res[
                    'Pedestrian_aos_all']
                cyc_3d, cyc_bev, cyc_aos = res['Cyclist_3d_all'], res['Cyclist_bev_all'], res['Cyclist_aos_all']

                mAP_3d = (car_3d + ped_3d + cyc_3d) / 3
                mAP_bev = (car_bev + ped_bev + cyc_bev) / 3
                mAP_aos = (car_aos + ped_aos + cyc_aos) / 3

                log_str += f"Car: {car_3d:.2f}, {car_bev:.2f}, {car_aos:.2f}\n"
                log_str += f"Ped: {ped_3d:.2f}, {ped_bev:.2f}, {ped_aos:.2f}\n"
                log_str += f"Cyc: {cyc_3d:.2f}, {cyc_bev:.2f}, {cyc_aos:.2f}\n"
                log_str += f"mAP: {mAP_3d:.2f}, {mAP_bev:.2f}, {mAP_aos:.2f}\n"

                prefix = f"VoD_{region_key}"
                ap_dict[f"{prefix}/Car_3d"] = car_3d
                ap_dict[f"{prefix}/Pedestrian_3d"] = ped_3d
                ap_dict[f"{prefix}/Cyclist_3d"] = cyc_3d
                ap_dict[f"{prefix}/mAP_3d"] = mAP_3d
                ap_dict[f"{prefix}/Car_bev"] = car_bev
                ap_dict[f"{prefix}/Pedestrian_bev"] = ped_bev
                ap_dict[f"{prefix}/Cyclist_bev"] = cyc_bev
                ap_dict[f"{prefix}/mAP_bev"] = mAP_bev
    ...
    def convert_valid_bboxes(self, box_dict, info):
        ...
        """
        if 'pts_bbox' in box_dict:
            box_dict = box_dict['pts_bbox']
        elif 'boxes_3d' in box_dict:
            pass
        else:

            possible_keys = ['pred_boxes3d', 'pred_boxes', 'prediction']
            for key in possible_keys:
                if key in box_dict:
                    box_dict = {'boxes_3d': box_dict[key],
                                'scores_3d': box_dict.get('scores', torch.ones(len(box_dict[key]))),
                                'labels_3d': box_dict.get('labels', torch.zeros(len(box_dict[key])))}
                    break
            else:
                raise ValueError(f"keys: {box_dict.keys()}")
