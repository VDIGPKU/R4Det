...
from mmdet3d.utils.visualization import draw_bev_pts_bboxes, colorize
import cv2   
...
class ImageAug3D2():
    ...
    def img_transform(self, img,my_gt_depth, post_rot, post_tran, resize, resize_dims, crop, flip, rotate):
        img = Image.fromarray(img.astype('uint8'), mode='RGB')
        img = img.resize(resize_dims)
        img_np_after_resize = np.array(img).astype(np.float32)
        img = img.crop(crop)
        img_np_after_crop = np.array(img).astype(np.float32)
        img_np_after_crop = np.array(img).astype(np.float32)
        if flip:
            img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
            img_np_after_flip = np.array(img).astype(np.float32)
        img = img.rotate(rotate)
        img_np_after_rotate = np.array(img).astype(np.float32)
        if my_gt_depth is not None:
            gt_depth_pil = Image.fromarray(my_gt_depth, mode='F')
            gt_depth_pil = gt_depth_pil.resize(resize_dims, resample=Image.NEAREST)
            gt_depth_pil = gt_depth_pil.crop(crop)
            if flip:
                gt_depth_pil = gt_depth_pil.transpose(method=Image.FLIP_LEFT_RIGHT)
            if rotate != 0:
                gt_depth_pil = gt_depth_pil.rotate(rotate, resample=Image.NEAREST, fillcolor=0)
            my_gt_depth = np.array(gt_depth_pil)
        ...
        post_rot = A.matmul(post_rot)
        post_tran = A.matmul(post_tran) + b
        return img,my_gt_depth, post_rot, post_tran
    
    def __call__(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if 'img' in data:
            initial_img = data['img']
        filepath = data.get('filename', 'Unknown Filename')
        sample_idx = data.get('sample_idx', 'N/A')
        ...
        img = data['img']
        img_np_input = np.array(img).astype(np.float32)
        my_gt_depth = data.get('my_gt_depth', None)
        img,my_gt_depth, post_rot2, post_tran2 = self.img_transform(
            img,
            my_gt_depth,
            post_rot, 
            post_trans, 
        ...
        data['my_gt_depth']=my_gt_depth


        img_np_output = np.array(img).astype(np.float32)
        ...
        if 'gt_masks' in data and data['gt_masks'] is not None and len(data['gt_masks'].masks) > 0:

            original_masks_np = data['gt_masks'].masks

            augmented_masks_list = []

            for i in range(original_masks_np.shape[0]):
                single_mask_np = original_masks_np[i]
                mask_pil = Image.fromarray(single_mask_np.astype(np.uint8) * 255)
                mask_pil = mask_pil.resize(resize_dims, resample=Image.NEAREST)
                mask_pil = mask_pil.crop(crop)
                if flip:
                    mask_pil = mask_pil.transpose(method=Image.FLIP_LEFT_RIGHT)
                if rotate != 0:
                    mask_pil = mask_pil.rotate(rotate, resample=Image.NEAREST, fillcolor=0)
                augmented_mask_np = (np.array(mask_pil) > 128).astype(np.uint8)
                augmented_masks_list.append(augmented_mask_np)

            if augmented_masks_list:
                final_masks_np = np.stack(augmented_masks_list, axis=0)
            else:
                fH, fW = self.final_dim
                final_masks_np = np.zeros((0, fH, fW), dtype=np.uint8)
            data['gt_masks'].masks = final_masks_np
            data['gt_masks'].height, data['gt_masks'].width = final_masks_np.shape[1:]
        ...
        old_h, old_w = data['ori_shape'][:2]
        new_h, new_w = data['img'].shape[:2]
        w_scale = new_w / old_w
        h_scale = new_h / old_h
